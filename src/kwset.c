...
#define udo_obstack_chunk_alloc kws_malloc
#define udo_obstack_chunk_free kws_free
...
kwset_t kwsalloc(char const *trans)
{
	...
	if (kwset != NULL)
	{
		if (!udo_obstack_init(&kwset->obstack))
		{
			free(kwset);
			return NULL;
		}
		...
		kwset->target = 0;
		kwset->trans = trans;
		kwset->trie = (struct trie *) udo_obstack_alloc(&kwset->obstack, sizeof(struct trie));
...
int kwsincr(kwset_t kws, char const *text, size_t len, void *user)
{
	...
	while (len--)
	{
		...
		if (!link)
		{
			link = (struct tree *) udo_obstack_alloc(&kwset->obstack, sizeof(struct tree));

			if (!link)
				return FALSE;
			link->llink = 0;
			link->rlink = 0;
			link->trie = (struct trie *) udo_obstack_alloc(&kwset->obstack, sizeof(struct trie));
...
int kwsprep(kwset_t kws)
{
	...
	if (kwset->words == 1 && kwset->trans == 0)
	{
		...
		kwset->target = (char *)udo_obstack_alloc(&kwset->obstack, kwset->mind);
...
void kwsfree(kwset_t kws)
{
	...
	if (kwset)
	{
		udo_obstack_free(&kwset->obstack, 0);
		free(kwset);
	}
}
