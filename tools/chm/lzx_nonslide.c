			 ...
			 get_chars_t get_chars, output_match_t output_match, output_literal_t output_literal, void *user_data)
{
	...
	lzi->block_buf = (uint8_t *)malloc(lzi->block_buf_size);
	...
	lzi->lentab = (int *)calloc(sizeof(int), lzi->block_buf_size);
	lzi->prevtab = (uint8_t **)calloc(sizeof(uint8_t *), lzi->block_buf_size);
...
int main(int argc, char *argv[])
{
	...
	lz_init(&lzi, wsize, wsize - 3, MAX_MATCH, MIN_MATCH, 8192, tmp_get_chars, tmp_output_match, tmp_output_literal, &lzu);
	lz_compress(&lzi);
	return 0;
}
#endif
...
int lz_compress(lz_info *lzi, int nchars)
{
	...
	{
#if 1
		if (!lzi->analysis_valid || (!lzi->eofcount && ((lzi->chars_in_buf - lzi->block_loc) < nchars)))
		{
			int residual = lzi->chars_in_buf - lzi->block_loc;
...
#endif
			memmove(lzi->block_buf, lzi->block_buf + lzi->chars_in_buf - bytes_to_move, bytes_to_move);
...
#else
		if (!lzi->analysis_valid || (lzi->block_loc - lzi->chars_in_buf) == 0)
		{
			lzi->block_loc = 0;
			lzi->chars_in_buf = 0;
			fill_blockbuf(lzi, nchars);
		...
		while ((bbp < bbe) && (!lzi->stop))
		{
			...
			if (lzi->frame_size && (len > (lzi->frame_size - lzi->cur_loc % lzi->frame_size)))
			{
				...
				fprintf(stderr, "Trim for framing: %06x %d %d\n", lzi->cur_loc, len,
						(lzi->frame_size - lzi->cur_loc % lzi->frame_size));
#endif
				trimmed = 1;
...
#endif
				if (lzi->output_match(lzi, (*prevp - lzi->block_buf) - lzi->block_loc, len) < 0)
				{
