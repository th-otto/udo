...
#include <math.h>
#ifdef NONSLIDE
#include "lzx_nonslide.h"
#else
#include "lzx_hash_slide.h"
#include "lzx_slide.h"
#endif
...
static short num_position_slots[] = { 30, 32, 34, 36, 38, 42, 50 };

static unsigned long position_base[51];

static uint8_t extra_bits[52];

static double rloge2;
...
{
	const struct h_elem *a = (const struct h_elem *)in_a;
	const struct h_elem *b = (const struct h_elem *)in_b;
...
{
	h_elem *leaves = (h_elem *)malloc(nelem * sizeof(h_elem));
	ih_elem *inodes;
	ih_elem *next_inode;
	ih_elem *cur_inode;
	h_elem *cur_leaf;
	...
	{
#ifdef DEBUG_HUFFMAN
		fprintf(stderr, "%3d: %3d '%c'\n", leaves_left, leaves[leaves_left].freq, leaves[leaves_left].sym);
#endif
		if (!leaves[leaves_left].freq)
			break;
	}
	nleaves = leaves_left;
	...
	if (nleaves >= 2)
	{
		inodes = (ih_elem *)malloc((nelem - 1) * sizeof(ih_elem));
		do
		{
			if (codes_too_long)
			{
			...
			{
				f1 = f2 = NULL;
				if (leaves_left && ((cur_inode == next_inode) || (cur_leaf->freq <= cur_inode->freq)))
				{
					f1 = (ih_elem *) cur_leaf++;
					leaves_left--;
		...
		while (cur_inode < next_inode)
		{
			fprintf(stderr, "%d l: %3d%c  r: %3d%c  freq: %8d\n",
					cur_inode - inodes,
					(cur_inode->left->sym !=
					 -1) ? (((struct h_elem *) cur_inode->left) - leaves) : (cur_inode->left - inodes),
					(cur_inode->left->sym != -1) ? 'l' : 'i',
					(cur_inode->right->sym !=
					 -1) ? (((struct h_elem *) cur_inode->right) - leaves) : (cur_inode->right - inodes),
					(cur_inode->right->sym != -1) ? 'l' : 'i', (cur_inode->freq));
			cur_inode++;
		}
#endif
		...
		while (cur_inode < next_inode)
		{
			fprintf(stderr, "%d l: %3d%c  r: %3d%c  freq: %8d  pathlength %4d\n",
					...
					(cur_inode->right->sym !=
					 -1) ? (((struct h_elem *) cur_inode->right) - leaves) : (cur_inode->right - inodes),
					(cur_inode->right->sym != -1) ? 'l' : 'i', (cur_inode->freq), (cur_inode->pathlength));
			cur_inode++;
		}
#endif
		free(inodes);
...
static int lzx_get_chars(lz_info *lzi, int n, uint8_t *buf)
{
	...
	if ((chars_read < n) && (lzud->left_in_frame))
	{
		...
		if (chars_pad > lzud->left_in_frame)
			chars_pad = lzud->left_in_frame;
		...
		if (chars_pad == LZX_FRAME_SIZE)
			chars_pad = 0;
#ifdef OLDFRAMING
		if (chars_pad > lzud->left_in_block)
			chars_pad = lzud->left_in_block;
#endif
...
static int find_match_at(lz_info *lzi, int loc, int match_len, int *match_locp)
{
	...
	if (matchb < lzi->slide_buf)
		matchb += lzi->slide_buf_size;
	nmatchb = lzi->slide_bufp - loc;
	if (nmatchb < lzi->slide_buf)
		nmatchb += lzi->slide_buf_size;
	c1 = matchb;
	c2 = nmatchb;
	for (j = 0; j < match_len; j++)
	{
		if (*c1++ != *c2++)
			break;
		if (c1 == lzi->slide_bufe)
			c1 = lzi->slide_buf;
		if (c2 == lzi->slide_bufe)
			c2 = lzi->slide_buf;
	}
	if (j == match_len)
	{
...
static void check_entropy(lzx_data *lzud, int main_index)
{
	...
	if (((n & 0xFFF) == 0) && (lzud->left_in_block >= 0x1000))
	{
		...
		cur_ratio =
			(n * rn_ln2 * (n_ln_n + lzud->main_entropy) + 24 + 3 * 80 + NUM_CHARS +
			 (lzud->main_tree_size - NUM_CHARS) * 3 + NUM_SECONDARY_LENGTHS) / (double) n;
#ifdef DEBUG_ENTROPY
		fprintf(stderr, "n = %d\n", n);
  ...
  testforr:
	...
	} else
	{
		...
		if ((match_len < 3) ||
			...
			((formatted_offset >= 2048) && (match_len < 5)) || ((formatted_offset >= 65536) && (match_len < 6)))
		{
		...
		} else
		{
			...
			{
				mid = (left + right) / 2;
				if ((position_base[mid] <= formatted_offset) && position_base[mid + 1] > formatted_offset)
				{
					position_slot = mid;
					break;
				}
				...
				fprintf(stderr, "BEFORE: %06x %06x %06x %06x\n",
						position_base[left], position_base[mid], formatted_offset, position_base[right]);
#endif
			...
			if (position_slot < 0)
			{
				...
				fprintf(stderr, "AFTER : %07d %07d %07d %07d\n",
						position_base[left], position_base[mid], formatted_offset, position_base[right]);
				fprintf(stderr, "(%d, %d, %d, %d, %d)\n", match_pos, match_len, formatted_offset, position_slot,
						position_footer);
			}
#endif
...
#ifdef DEBUG_MATCHES
#ifdef NONSLIDE
	fprintf(stderr, "(%08x, %d, %d, %d, %d, %d)\n", lzud->lzi->cur_loc, match_pos, match_len, formatted_offset,
			position_slot, position_footer);
#else
	fprintf(stderr, "(%08x, %d, %d, %d, %d, %d)\n", lzud->lzi->cur_loc - lzud->lzi->chars_in_match, match_pos,
			match_len, formatted_offset, position_slot, position_footer);
#endif
#endif
	/* match length = 8 bits */
...
static void lzx_output_literal(lz_info *lzi, uint8_t ch)
{
	lzx_data *lzud = (lzx_data *) lzi->user_data;
...
#endif
		bytebuf[0] = lzxd->bit_buf & 0xFF;
		bytebuf[1] = (lzxd->bit_buf >> 8) & 0xFF;
		lzxd->put_bytes(lzxd->out_arg, 2, bytebuf);
		lzxd->len_compressed_output += 2;
		lzxd->bit_buf = 0;
		nbits -= shift_bits;
		cur_bits = 0;
	}
...
static void lzx_write_compressed_literals(lzx_data *lzxd, int block_type)
{
	...
	while (cursor < endp)
	{
		...
		if (frame_count > LZX_FRAME_SIZE)
		{
			fprintf(stderr, "uncomp_len = %x, frame_count = %x, block_code = %08x, match_len_m2 = %d",
					lzxd->len_uncompressed_input, frame_count, block_code, match_len_m2);
		}
#endif
...
static int lzx_write_compressed_tree(struct lzx_data *lzxd, struct huff_entry *tree, uint8_t *prevlengths, int treesize)
{
	...
	codep = codes = (uint8_t *)malloc(treesize * sizeof(char));
	runp = runs = (uint8_t *)malloc(treesize * sizeof(char));
...
int lzx_compress_block(lzx_data *lzxd, int block_size, int subdivide)
{
	...
	{
		if (lzxd->block_codes != NULL)
			free(lzxd->block_codes);
		lzxd->block_size = block_size;
		lzxd->block_codes = (uint32_t *)malloc(block_size * sizeof(uint32_t));
	}
	...
	do
	{
			...
			(!lz_left_to_process(lzxd->lzi) && lzxd->at_eof(lzxd->in_arg)))
		{
			...
			/* end extra bits */
			build_huffman_tree(lzxd->main_tree_size, LZX_MAX_CODE_LENGTH, lzxd->main_freq_table, lzxd->main_tree);
			build_huffman_tree(NUM_SECONDARY_LENGTHS, 16, lzxd->length_freq_table, lzxd->length_tree);
			...
			lzx_write_compressed_tree(lzxd, lzxd->main_tree + NUM_CHARS,
									  lzxd->prev_main_treelengths + NUM_CHARS, lzxd->main_tree_size - NUM_CHARS);

			/* now write the pre tree and tree for length */
			lzx_write_compressed_tree(lzxd, lzxd->length_tree, lzxd->prev_length_treelengths, NUM_SECONDARY_LENGTHS);
			 ...
			 lzx_mark_frame_t mark_frame, void *mark_frame_arg)
{
	...
	*lzxdp = lzxd = (struct lzx_data *)malloc(sizeof(*lzxd));
	if (lzxd == 0)
		return -2;
	...
	lzxd->main_freq_table = (int *)malloc(sizeof(int) * lzxd->main_tree_size);
	lzxd->main_tree = (huff_entry *)malloc(sizeof(huff_entry) * lzxd->main_tree_size);
	lzxd->prev_main_treelengths = (uint8_t *)malloc(sizeof(uint8_t) * lzxd->main_tree_size);

	lzxd->lzi = (lz_info *)malloc(sizeof(*lzxd->lzi));
#ifdef NONSLIDE
...
#else
	lz_init(lzxd->lzi, wsize, MAX_MATCH, MIN_MATCH, LZX_FRAME_SIZE,
			lzx_get_chars, lzx_output_match, lzx_output_literal, lzxd);
#endif
